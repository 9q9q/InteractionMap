...
class DataBase(object):
    def __init__(self, name, path, batch_size, shuffle=False, reshaped_size=None, img_size=None, gray_scale=False,
        ...
        if name == 'from_ImageFolder':
            ...
            if reshaped_size is not None:
                list_of_transforms.append(
                    transforms.Resize(size=reshaped_size))
            ...
            if do_whitening:
                if whitening_params is not None:
                    list_of_transforms.append(
                        whitening(img_size, **whitening_params))
                else:
        ...
        if name == 'from_tensor':
            ...
            data_set = MyImageLoader(
                root=path, transform=transform, retun_idx=return_idx)
        elif name == 'CIFAR10_tr':
            if normalize == True:
                list_of_transforms.append(transforms.Normalize(
        ...
        if data_set.__len__() % self.batch_size != 0:
            ...
            self.data = torch.utils.data.DataLoader(
                data_set, batch_size=batch_size, shuffle=shuffle, num_workers=num_workers, drop_last=True)
        else:
            self.data = torch.utils.data.DataLoader(
                data_set, batch_size=batch_size, shuffle=shuffle, num_workers=num_workers)
...
class LCN(object):
    ...
    def __init__(self, kernel_size=9, sigma=0.5, rgb=False, param_shit=1):
        ...
        x = torch.from_numpy(np.linspace(-1, 1, kernel_size)
                             ).unsqueeze(1).expand(kernel_size, kernel_size).float()
        y = torch.from_numpy(np.linspace(-1, 1, kernel_size)
                             ).unsqueeze(0).expand(kernel_size, kernel_size).float()
        r_2 = x.pow(2) + y.pow(2)
        self.kernel_size = kernel_size
        self.gaussian_k = torch.exp(-r_2/sigma).unsqueeze(0).unsqueeze(0)
        if rgb:
    ...
    def __call__(self, img):
        # subtractive step
        img = img.unsqueeze(0)
        img_pad = F.pad(img, ((self.kernel_size - 1) // 2, (self.kernel_size - 1) //
                        2, (self.kernel_size - 1) // 2, (self.kernel_size - 1) // 2))
        gaus_map = F.conv2d(img_pad, self.gaussian_k)
        img_sub = img - gaus_map
        # divisive step
        img_pad = F.pad(img_sub, ((self.kernel_size - 1) // 2, (self.kernel_size - 1) //
                        2, (self.kernel_size - 1) // 2, (self.kernel_size - 1) // 2))
        img_sigma = F.conv2d(img_pad.pow(2), self.gaussian_k).sqrt()
        c = img_sigma.view(img_sigma.size()[
                           0], -1).mean(-1, keepdim=True).unsqueeze(-1).unsqueeze(-1).expand_as(img_sigma)
        img_sigma = (F.relu(img_sigma - c) > 0).float() * \
            img_sigma + (1 - (F.relu(img_sigma - c) > 0).float()) * c
        img_div = img_sub / img_sigma
...
class whitening(object):
    ...
    def __init__(self, img_size, f_0=0.5, n=4):
        ...
        f_x = torch.from_numpy(np.linspace(-0.5, 0.5, dim_x)
                               ).unsqueeze(1).expand(dim_x, dim_y).float()
    ...
    def __call__(self, img):
        ...
        # apparently equivalent (https://stackoverflow.com/questions/71515439/equivalent-to-torch-rfft-in-newest-pytorch-version)
        img_f = torch.view_as_real(torch.fft.fft2(img))
        filt = self.f * torch.exp(-(self.f / self.f_0).pow(self.n))
        img_f_ = img_f * filt
        # img = torch.irfft(img_f_, 2, onesided=False)
        img_f = torch.view_as_real(torch.fft.ifft2(img_f_))
...
def to_img(x):
    val_max = x.abs().max(-1,
                          keepdim=True)[0].max(-2, keepdim=True)[0].expand_as(x)
    x += val_max
    x /= 2 * val_max
    x.clamp(0, 1)
    return x

...
class Monitor(object):
    def __init__(self, Net, writer, n_row=None, DicoL=True):
        ...
        if torch.cuda.is_available():
            self.total_activity = [torch.zeros(
                1, 1, 1, 1).cuda()]*self.Net.nb_layers
            self.act = [torch.zeros(1, self.Net.layers[i].dico_shape[0], 1, 1).cuda(
            ) for i in range(self.Net.nb_layers)]
        else:
            self.total_activity = [torch.zeros(
                1, 1, 1, 1)] * self.Net.nb_layers
            self.act = [torch.zeros(1, self.Net.layers[i].dico_shape[0], 1, 1)
                        for i in range(self.Net.nb_layers)]
        ...
        if DicoL:
            ...
            for i in range(Net.nb_layers - 1):
                if self.Net.layers[i].transform is not None:
                    ...
                    stride = 1
                self.all_stride_list.append(
                    self.all_stride_list[-1] * stride * self.Net.layers[i+1].stride)
            if torch.cuda.is_available():
                self.size_eff = [self.Net.project_dico(
                    i, cpu=False).size() for i in range(self.Net.nb_layers)]
                self.D_eff = [torch.randn(self.size_eff[i]).cuda()
                              for i in range(self.Net.nb_layers)]
                self.D_eff_m = [torch.randn(self.size_eff[i]).cuda()
                                for i in range(self.Net.nb_layers)]
            else:
                self.size_eff = [self.Net.project_dico(
                    i, cpu=True).size() for i in range(self.Net.nb_layers)]
                self.D_eff = [torch.randn(self.size_eff[i])
                              for i in range(self.Net.nb_layers)]
                self.D_eff_m = [torch.randn(self.size_eff[i])
                                for i in range(self.Net.nb_layers)]
        ...
        for idx_layer in range(self.Net.nb_layers):
            if separate:
                self.writer.add_scalar(
                    '5a_LL/L{0}'.format(idx_layer), loss.value[idx_layer][0], k)
                self.writer.add_scalar(
                    '5b_UL/L{0}'.format(idx_layer), loss.value[idx_layer][1], k)
            else:
    ...
    def MonitorGamma(self, gamma, k, option=['NNZ', '%', 'Mean', 'Sum', 'V']):
        for i in range(self.Net.nb_layers):
            active_els = (gamma[i] != 0).float().sum().div(
                self.Net.sparse_map_size[i][0])
            if 'NNZ' in option:
                self.writer.add_scalar(
                    '1_Sparsity_NNZ/L{}'.format(i), active_els, k)

            volume = float(
                (gamma[i].size()[-1] * gamma[i].size()[-2] * gamma[i].size()[-3]))
            relative_sparsity = 100 - (active_els.float() / volume) * 100

            if '%' in option:
                self.writer.add_scalar(
                    '0_Sparsity_Pct/L{}'.format(i), relative_sparsity, k)

            if 'Mean' in option:
                avg = torch.mean(
                    self.Net.layers[i].v * gamma[i][gamma[i] != 0])
                self.writer.add_scalar(
                    '2_Mean_Activity/L{0}'.format(i), avg, k)
            ...
            if 'V' in option:
                if self.Net.layers[i].v_size is not None:
                    self.writer.add_scalar(
                        '3_V/L{}'.format(i), self.Net.layers[i].v.detach().mean(), k)

    def MonitorDicoBP(self, k):
        for i in range(self.Net.nb_layers):
            name = 'DicoBP/L_{0}'.format(i+1)
            dico_img = make_grid(self.Net.project_dico(
                i), normalize=True, pad_value=1, nrow=self.n_row[i])
        ...
        for i in range(self.Net.nb_layers):
            ...
            for idx in range(nb_iter):
                ...
                loss_D_eff = (batch - f.conv_transpose2d(gamma[i], self.D_eff[i], stride=self.all_stride_list[i],
                                                         padding=self.Net.layers[i].pad *
                                                         self.all_stride_list[i],
            ...
            dico_img = make_grid(
                self.D_eff[i], normalize=True, pad_value=1, nrow=self.n_row[i])
    ...
    def ComputeHisto(self, gamma):
        ...
        for i in range(self.Net.nb_layers):
            ...
            self.act[i] += gamma[i].abs().sum(0, keepdim=True).sum(-1,
                                                                   keepdim=True).sum(-2, keepdim=True)

    def Save(self, file_name):
        to_save = {'dico_eff': [self.D_eff[i].cpu()
                                for i in range(self.Net.nb_layers)]}
